...
begin
  ...
  lock table public.seds, public.llaves, public.fallas, public.fault_periods, public.sed_monthly_metrics, public.circuit_monthly_metrics in share row exclusive mode nowait;
  ...
  delete from public.circuit_monthly_metrics where sed_id is not null and llave_code is not null and period_key is not null;
  ...
  update public.fault_periods p set row_count = counts.row_count
  from (select period_key, count(*)::bigint row_count from public.fallas where period_key is not null group by period_key) counts
...
begin
  ...
  delete from public.llaves where id is not null;
  delete from public.seds where id is not null;
  if exists (select 1 from public.seds)
    or exists (select 1 from public.llaves)
    or exists (select 1 from public.fallas)
    or exists (select 1 from public.fault_periods)
    or exists (select 1 from public.sed_monthly_metrics)
    or exists (select 1 from public.circuit_monthly_metrics)
