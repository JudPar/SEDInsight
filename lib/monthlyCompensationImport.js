...
export function prepareMonthlyCircuitCompensationImport(input, permanentCircuits, existingRows = []) {
  ...
  rows.forEach((row, index) => {
    ...
    if (!circuitKey || !periodMonths.length || !compensation.valid) return invalid.push({ index, row });
    ...
    seen.add(rowKey);
    accepted.push({ ...circuit, period_key: periodKey, period_end_key: periodEndKey, compensation: compensation.value });
  });
