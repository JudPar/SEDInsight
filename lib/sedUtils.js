...
/**
 * Canonical identifier used for joins and metrics. It intentionally preserves
 * leading zeroes and does not try to extract a SED from a SED-Llave value.
 */
export function normalizeSedId(value) {
...
export function normalizeLlaveCode(value) {
  if (value === null || value === undefined) return '';
  return String(value)
    .normalize('NFC')
    .replace(/[\u200B-\u200D\u2060\uFEFF]/g, '')
...
export function normalizeLlaveSuffix(value) {
  const suffix = normalizeLlaveCode(value).split('/').pop()?.replace(/\s+/g, '') || '';
  const match = suffix.match(/^0*(\d+)([A-Z]+)$/);
  return match ? `${Number(match[1])}${match[2]}` : suffix;
}

function llaveCompatibilityKey(value) {
  const suffix = normalizeLlaveSuffix(value);
  const match = suffix.match(/^(\d+)(SP|S)$/);
  return match ? `${match[1]}S` : suffix;
}

export function areLlaveCodesEquivalent(left, right, knownLlaveCodes = []) {
  const leftSuffix = normalizeLlaveSuffix(left);
  const rightSuffix = normalizeLlaveSuffix(right);
  if (!leftSuffix || !rightSuffix) return false;
  if (leftSuffix === rightSuffix) return true;
  if (llaveCompatibilityKey(leftSuffix) !== llaveCompatibilityKey(rightSuffix)) return false;
  const compatibleKnown = [...new Set((knownLlaveCodes || [])
    .map(normalizeLlaveSuffix)
    .filter(Boolean)
    .filter(code => llaveCompatibilityKey(code) === llaveCompatibilityKey(rightSuffix)))];
  return compatibleKnown.length === 1 && compatibleKnown[0] === rightSuffix;
}

export function canonicalCircuitKey(sedId, llaveCode) {
  const normalizedSedId = normalizeSedId(sedId);
  const normalizedLlaveCode = normalizeLlaveCode(llaveCode);
  return normalizedSedId && normalizedLlaveCode ? `${normalizedSedId}\u0000${normalizedLlaveCode}` : '';
}
 ...
 * @returns {boolean}
 */
export function isLlaveMatch(fallaLlaveCode, fallaSedLlave, targetLlaveId, knownLlaveCodes = []) {
  ...
  if (fallaLlaveCode) {
    ...
    if (rawLlave === rawTarget || cleanLlave === cleanTarget || areLlaveCodesEquivalent(cleanLlave, cleanTarget, knownLlaveCodes)) return true;
  }
  ...
  if (fallaSedLlave) {
    ...
    if (parts.length > 1) {
      ...
      const cleanPart = llavePart.includes('/')
        ...
        : llavePart;
      if (llavePart === rawTarget || cleanPart === cleanTarget || areLlaveCodesEquivalent(cleanPart, cleanTarget, knownLlaveCodes)) return true;
    }
  }
