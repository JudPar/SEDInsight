...
function summarizeCircuitCompensation(rows, sedId, llaveCode, selectedPeriodKeys, circuitFaults) {
  ...
  const circuitFaultsCompatible = faultCountByPeriod.filter(period => compatibleSet.has(period.periodKey)).reduce((total, period) => total + period.count, 0);
...
function prioritizeMonthlyCompensation(circuit, sed, selectedPeriodKeys) {
  const circuitPeriods = new Map(circuit.byPeriod.map(period => [period.periodKey, period]));
  const sedPeriods = new Map((sed.byPeriod || []).map(period => [period.periodKey, period]));
  const circuitFaults = new Map((circuit.faultCountByPeriod || []).map(period => [period.periodKey, period.count]));
  const sedFaults = new Map((sed.faultCountByPeriod || []).map(period => [period.periodKey, period.count]));
  const byPeriod = selectedPeriodKeys.map((periodKey) => {
    const circuitPeriod = circuitPeriods.get(periodKey);
    const sedPeriod = sedPeriods.get(periodKey);
    if (circuitPeriod?.available) return { periodKey, value: circuitPeriod.value, available: true, sourceScope: 'circuit', faultsCompatible: circuitFaults.get(periodKey) || 0 };
    if (sedPeriod?.available) return { periodKey, value: sedPeriod.value, available: true, sourceScope: 'sed', faultsCompatible: sedFaults.get(periodKey) || 0 };
    return { periodKey, value: null, available: false, sourceScope: null, faultsCompatible: 0 };
  });
  ...
  return {
    ...
    totalKnown,
    dataAvailable: available.length > 0,
    dataComplete: selectedPeriodKeys.length > 0 && available.length === selectedPeriodKeys.length,
    coverageStatus: !available.length ? 'unavailable' : available.length === selectedPeriodKeys.length ? 'complete' : 'partial',
    compatiblePeriodKeys: available.map(period => period.periodKey),
    periodsMissing: byPeriod.filter(period => !period.available).map(period => period.periodKey),
    faultsCompatible,
    compensationPerFault: totalKnown !== null && faultsCompatible > 0 ? totalKnown / faultsCompatible : null,
