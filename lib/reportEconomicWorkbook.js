...
export function addEconomicWorkbook(workbook, model) {
  ...
  const sourceRows = [
    ...
    ['Compensación compatible', sim.compensationPerFault.compatibleCompensation, 'S/', `Ámbito: ${sim.compensationPerFault.sourceScope || 'sin dato'}`],
  ...
  f(6, 'Compensación por falla', `IF(${S(15)}="manual",IF(AND(ISNUMBER(${S(16)}),${S(16)}>=0),${S(16)},${NA}),IF(AND(ISNUMBER(${S(13)}),${S(13)}>=0,ISNUMBER(${S(14)}),${S(14)}>0),${S(13)}/${S(14)},${NA}))`, sim.compensationPerFault.value, 'S/falla', 'Modo manual o compensación / fallas compatibles', CURRENCY);
  ...
  f(12, 'Beneficio anual evitable', `IF(AND(ISNUMBER(B11),ISNUMBER(${S(17)}),${S(17)}>=0,${S(17)}<=1),B11*${S(17)},${NA})`, sim.annualAvoidedBenefit, 'S/año', 'Exposición × factor evitable', CURRENCY);
