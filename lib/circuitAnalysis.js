...
export function analyzeCircuitPhase1(linesData = []) {
  ...
  const segmentsWithMultipleCalibres = physicalSegments.filter(segment => segment.calibreResolution.manualLabels.length > 1).length;
  const calibreConflicts = physicalSegments.filter(segment => segment.calibreResolution.conflict).map(segment => ({
  ...
  const manualCalibreCandidateSegments = physicalSegments.filter(segment => segment.calibreResolution.manualLabels.length === 1).length;
  const structuredCalibreSegments = physicalSegments.filter(segment => segment.calibreResolution.source === 'structured').length;
  const manualCalibreSegments = physicalSegments.filter(segment => segment.calibreResolution.source === 'cableGroup').length;
  ...
  return {
    ...
    lengthByCalibre,
    segmentsWithoutCalibre: physicalSegments.length - resolvedCalibreSegments,
    structuredCalibreSegments,
    manualCalibreSegments,
    manualCalibreCandidateSegments,
    resolvedCalibreSegments,
    manualCalibreCoveragePercent: physicalSegments.length ? manualCalibreCandidateSegments / physicalSegments.length * 100 : 0,
    resolvedCalibreCoveragePercent: physicalSegments.length ? resolvedCalibreSegments / physicalSegments.length * 100 : 0,
    calibreConflicts,
